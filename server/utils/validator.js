...
export default {
    ...
    validateModel(model, rules) {
        ...
        for (let field in rules) {
            ...
            if (ruleTypes.indexOf('date_time') !== -1 && isNext) {
                if (value && !this.isValidDateTime(value)) {
                    isNext = false;
                    errors.push({ field, message: messages['date_time'].message });
                }
            }
        }
        return errors;
    ...
    /**
     * Check field is valid date time
     * 
     * @param {object} data    
     */
    isValidDateTime(data) {
        if (this.checkPattern(data, /^(\d{4})\-(\d{2})\-(\d{2}) (\d{2}):(\d{2}):(\d{2})$/)) {
            return true;
        }
        return false;
    },
